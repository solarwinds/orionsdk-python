# -*- coding: utf-8 -*-
"""ASyncSolarWinds Class Overview
...
import logging
from .asyncswisclient import ASyncSwisClient


class ASyncSolarWinds:
    ...
    def __init__(self, npm_server, username, password, logger=None):
        ...
        self.swis = ASyncSwisClient(npm_server, username, password)

    async def does_node_exist(self, node_name):
    ...
    async def get_node_id(self, node_name):
        ...
        node_id = await self.swis.query("SELECT NodeID, Caption FROM Orion.Nodes WHERE Caption = @caption",
                                  caption=node_name)
    ...
    async def get_node_uri(self, node_name):
        ...
        node_uri = await self.swis.query("SELECT Caption, Uri FROM Orion.Nodes WHERE Caption = @caption",
                                   caption=node_name)
    ...
    async def add_node_using_snmp_v3(self, node_name, ip_address, snmpv3_username, snmpv3_priv_method, snmpv3_priv_pwd,
        ...
        if not await self.does_node_exist(node_name):
            ...
            results = await self.swis.create('Orion.Nodes', **node_properties)
            self.logger.info("add_node - add node invoke results: %s", results)

            # Assign pollers to node.
            await self.attach_poller_to_node(node_name, 'N.Status.ICMP.Native')
            await self.attach_poller_to_node(node_name, 'N.Status.SNMP.Native', False)
            await self.attach_poller_to_node(node_name, 'N.ResponseTime.ICMP.Native')
            await self.attach_poller_to_node(node_name, 'N.ResponseTime.SNMP.Native', False)
            await self.attach_poller_to_node(node_name, 'N.Details.SNMP.Generic')
            await self.attach_poller_to_node(node_name, 'N.Uptime.SNMP.Generic')

    async def is_poller_attached_to_node(self, node_name, poller_name):
        ...
        net_object_id = str(await self.get_node_id(node_name))
        net_object = 'N:' + net_object_id

        results = await self.swis.query("SELECT PollerType FROM Orion.Pollers WHERE NetObject = @net_object AND PollerType "
    ...
    async def attach_poller_to_node(self, node_name, poller_name, enabled=True):
        ...
        if not await self.is_poller_attached_to_node(node_name, poller_name):
            net_object_id = str(await self.get_node_id(node_name))
            net_object = 'N:' + net_object_id
            ...
            results = await self.swis.create('Orion.Pollers', **poller_properties)
            self.logger.info("attach_poller_to_node - poller create results: %s", results)

    async def enable_hardware_health_on_node(self, node_name):
        ...
        results = await self.swis.invoke('Orion.HardwareHealth.HardwareInfo', 'EnableHardwareHealth', net_object, 9)
        self.logger.info("enable_hardware_health - enable hardware health invoke results: %s", results)

    async def add_node_to_ncm(self, node_name):
        ...
        results = await self.swis.invoke('Cirrus.Nodes', 'AddNodeToNCM', self.get_node_id(node_name))
        self.logger.info("add_node_to_ncm - add node to ncm invoke results: %s", results)

    async def add_node_to_udt(self, node_name):
        udt_properties = {
            'NodeID': await self.get_node_id(node_name),
            'Capability': '2',
            'Enabled': True
        }

        results = await self.swis.create('Orion.UDT.NodeCapability', **udt_properties)
    ...
    async def add_node_to_ip_vnqm(self, node_name):
        ...
        results = await self.swis.create('Orion.IpSla.Sites', **vnqm_node_properties)
        self.logger.info("add_node_to_vnqm - add node to vnqm create results: %s", results)

    async def add_icmp_echo_ip_sla_operation_to_node(self, node_name, ip_sla_operation_number, ip_sla_name):
        ip_sla_properties = {
            'NodeID': await self.get_node_id(node_name),
            'OperationTypeID': 5,
        ...
        results = await self.swis.create('Orion.IpSla.Operations', **ip_sla_properties)
        self.logger.info("add_icmp_echo_ip_sla_operation_to_node - add IP SLA operation to node create results: %s", results)

    async def add_group_custom_property(self, property_name, description, value_type, size):
        ...
        results = await self.swis.invoke('Orion.GroupCustomProperties', 'CreateCustomProperty', property_name, description,
                                   value_type, size, "", "", "", "", "", "")
        self.logger.info("add_group_custom_property - add group custom property results: %s", results)

    async def set_group_custom_property(self, group_name, custom_property_name, custom_property_value):
        ...
        group_uri = await self.get_group_uri(group_name)

        custom_property = {
            custom_property_name: custom_property_value
        }

        await self.swis.update(group_uri + '/CustomProperties', **custom_property)
        
    async def set_node_custom_property(self, node_name, custom_property_name, custom_property_value):
        ...
        node_uri = await self.get_node_uri(node_name)

        custom_property = {
            custom_property_name: custom_property_value
        }

        await self.swis.update(node_uri + '/CustomProperties', **custom_property)

    async def get_node_custom_properties(self, node_name):
        ...
        custom_properties = await self.swis.read(node_uri + '/CustomProperties')
        self.logger.info("set_custom_properties - custom_properties read results: %s", custom_properties)

        return custom_properties

    async def get_list_of_custom_pollers_for_node(self, node_name):
        ...
        custom_pollers = await self.swis.query("SELECT CustomPollerName FROM Orion.NPM.CustomPollerAssignment WHERE NodeID "
    ...
    async def remove_custom_poller_by_name(self, node_name, poller_name):
        ...
        custom_poller_id = await self.swis.query("SELECT CustomPollerID FROM Orion.NPM.CustomPollers WHERE UniqueName = "
        ...
        custom_poller_uri = await self.swis.query("SELECT Uri FROM Orion.NPM.CustomPollerAssignmentOnNode WHERE "
        ...
        await self.swis.delete(custom_poller_uri['results'][0]['Uri'])

    async def add_custom_poller_by_name(self, node_name, poller_name):
        ...
        custom_poller_id = await self.swis.query(
        ...
        await self.swis.create('Orion.NPM.CustomPollerAssignmentOnNode', **poller_properties)

    async def does_group_exist(self, group_name):
        ...
        if await self.get_group_id(group_name):
            return True
        else:
            return False
    ...
    async def get_group_id(self, group_name):
        ...
        group_id = await self.swis.query("SELECT ContainerID FROM Orion.Container WHERE Name = @group_name",
    ...
    async def get_group_uri(self, group_name):
        ...
        group_uri = await self.swis.query("SELECT Uri FROM Orion.Container WHERE Name = @group_name",
                                    group_name=group_name)
    ...
    async def add_group(self, group_name, owner='Core', refresh_frequency=60, status_rollup=0,
        ...
        if group_members is None:
            group_members = []
        if not await self.does_group_exist(group_name):
            results = await self.swis.invoke('Orion.Container', 'CreateContainer', group_name, owner, refresh_frequency,
    ...
    async def is_node_in_group(self, node_name, group_name):
        ...
        results = await self.swis.query("SELECT Name FROM Orion.ContainerMembers WHERE ContainerID = @group_id and FullName "
    ...
    async def add_node_to_group(self, node_name, group_name):
        ...
        if not await self.is_node_in_group(node_name, group_name):
    ...
    async def delete_group_by_id(self, group_id):
        ...
        await self.swis.invoke('Orion.Container', 'DeleteContainer', int(group_id))
    ...
    async def delete_group_by_name(self, group_name):
        ...
        await self.delete_group_by_id(group_id)

    async def does_dependency_exist(self, dependency_name):
        ...
        if await self.get_dependency_id(dependency_name):
            return True
        else:
            return False

    async def get_dependency_id(self, dependency_name):
        ...
        dependency_id = await self.swis.query("SELECT DependencyId FROM Orion.Dependencies WHERE Name = @dependency_name",
    ...
    async def add_dependency(self, dependency_name, parent_name, child_name):
        ...
        if not await self.does_dependency_exist(dependency_name):
            if await self.does_node_exist(parent_name):
                self.logger.info("add-dependency - The parent is a node.")
                parent_id = await self.get_node_id(parent_name)
                parent_uri = await self.get_node_uri(parent_name)
                parent_entity_type = 'Orion.Nodes'
            elif await self.does_group_exist(parent_name):
                self.logger.info("add-dependency - The parent is a group.")
                parent_id = await self.get_group_id(parent_name)
                parent_uri = await self.get_group_uri(parent_name)
                parent_entity_type = 'Orion.Groups'
            else:
                return False

            if await self.does_node_exist(child_name):
                self.logger.info("add-dependency - The child is a node.")
                child_id = await self.get_node_id(child_name)
                child_uri = await self.get_node_uri(child_name)
                child_entity_type = 'Orion.Nodes'
            elif await self.does_group_exist(child_name):
                self.logger.info("add-dependency - The child is a group.")
                child_id = await self.get_group_id(child_name)
                child_uri = await self.get_group_uri(child_name)
            ...
            await self.swis.create('Orion.Dependencies', **dependency_properties)

    async def get_list_of_interfaces(self, node_name):
        ...
        list_interfaces_names = await self.swis.query("SELECT Name FROM Orion.NPM.Interfaces WHERE NodeID "
                                                "= @node_id", node_id=node_id)
    ...
    async def remove_interface(self, node_name, interface_name):
        ...
        interface_uri = await self.get_interface_uri(node_name, interface_name)

        if await self.does_interface_exist(node_name, interface_name):
            self.logger.info("remove_interface_by_name - interface_uri query results: %s", interface_uri)
            await self.swis.delete(interface_uri)
            return True
        else:
            return False

    async def get_interface_uri(self, node_name, interface_name):
        ...
        interface_uri = await self.swis.query(
    ...
    async def get_interface_id(self, node_name, interface_name):
        ...
        interface_id = await self.swis.query("SELECT InterfaceID FROM Orion.NPM.Interfaces WHERE NodeID=@node_id AND "
    ...
    async def does_interface_exist(self, node_name, interface_name):
        ...
        if await self.get_interface_id(node_name, interface_name):
            return True
        else:
            return False

    async def get_discovered_interfaces(self, node_name):
        ...
        discovered_interfaces = await self.swis.invoke('Orion.NPM.Interfaces', 'DiscoverInterfacesOnNode', node_id)

        return discovered_interfaces['DiscoveredInterfaces']

    async def add_interface(self, node_name, interface_name):
        ...
        discovered_interfaces = await self.get_discovered_interfaces(node_name)
        ...
        if discovered_interface:
            await self.swis.invoke('Orion.NPM.Interfaces',
                             'AddInterfacesOnNode',
                             node_id,
                             discovered_interface,
                             'AddDefaultPollers')
    ...
    async def ncm_download_nodes_running_config(self, node_name):
        ...
        results = await self.swis.query('SELECT NodeID FROM Cirrus.Nodes WHERE NodeCaption = @node_name',
                                  node_name=node_name)['results']

        cirrus_node_id = results[0]['NodeID']

        await self.swis.invoke('Cirrus.ConfigArchive', 'DownloadConfig', [cirrus_node_id], 'Running')

    async def ncm_run_compliance_report(self, report_name):
        ...
        results = await self.swis.query('SELECT PolicyReportID FROM Cirrus.PolicyReports WHERE Name = @report_name',
                                  report_name=report_name)

        report_id = results['results'][0]['PolicyReportID']
        await self.swis.invoke('Cirrus.PolicyReports', 'StartCaching', [report_id])

    async def add_dns_a_record_with_ptr_to_ipam(self, name, ip_address, server, domain):
        ...
        await self.swis.invoke('IPAM.IPAddressManagement', 'AddDnsARecordWithPtr', name, ip_address, server, domain)
